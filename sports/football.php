...
<?php 
    ...
    $footballM = $conn->query("SELECT * FROM matchs WHERE sport_id =  1 AND date >= CURRENT_TIMESTAMP");
    $footballM->execute();
    $row = $footballM->fetchAll();
...
<title>BetGuess - Football</title>

<html>
    <body>
        <center>    
        <br><h3>Football!</h3> <br>

        <h5>Paris sur les matchs du championnat de Ligue 1.<br><br>Selectionne tes pronostiques pour les matchs du jour.</h5><br>

        <?php foreach($row as $matchfoot) {
            $id = $matchfoot['id'];
            $domicile = $matchfoot['dom'];
            $exterieur = $matchfoot['ext']; ?>

                    <div class="container">                 <!-- Affiche les bouttons pour les matchs qui arrive -->
                        <div class="row justify-content-center">
                            <div class="box-foot" id="dom">
